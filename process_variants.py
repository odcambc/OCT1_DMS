...
variantCounts_colnames = ["counts", "coverage", "mean_length", "length_NT", "NT", "length_codon", "codon", "AA",
                          "mutations"]
...
def read_gatk_csv(file):
    # GATK output unfortunately may not be full-width, which will confuse pandas during reading in.
    # We'll read each line and make sure it's full-length before passing it to pandas.
    gatk_list = []
...
def process_variants_file(gatk_list, designed_variants_df):
    ...
    for line in gatk_list:
        ...
        # If a variant is mapped outside of the ORF, nucleotide change length is 0 and codon/AA fields are empty

        if length_NT == 0:
            pass

        # Consider how many discontiguous non-synonymous mutations are found. If more than 1, then this is not
        # a designed variant. If none are found, this is a synonymous variant.
        ...
        if len(mutation.split(";")) == 1:
            ...
            if mutation_type == "M" or mutation_type == "S":
                try:
                    designed_codon = variants_df[(variants_df['pos'] == pos)
                                                 & (variants_df['mutation'] == variant)]['codon'].array[0]
            ...
            insdel_re = regex.search(r'([a-zA-Z])([0-9]+)_([a-zA-Z])([0-9]+)insdel([a-zA-Z]+)', mutation)
            ...
            if insdel_re:
                ...
                if (insdel_re.group(1) == insdel_re.group(5) or insdel_re.group(3) == insdel_re.group(
                        5) or insdel_re.group(1) == insdel_re.group(3)):
                    if length - insdel_length < 5:
                        ...
                        try:
                            name = variants_df[(variants_df['pos'] == pos)
                                               & (variants_df['mutation'] == variant)]['name'].array[0]
...
def write_enrich_df(file, variant_df):
    wt_summed = variant_df[variant_df["mutation_type"] == "S"].sum()["count"]
...
def remove_zeros_enrich(enrich_file_list):
    ...
    df_list = [pd.read_csv(f, sep='\t', names = ['hgvs', f], header=0).set_index('hgvs') for f in enrich_file_list]

    combined_enrich_df = pd.concat(df_list, join='outer', axis=1)

    unobserved_variants = combined_enrich_df[(combined_enrich_df == 0).all(axis=1)].index.to_list()
    ...
    for enrich_file in enrich_file_list:
        with open(enrich_file, 'w+') as f:
            combined_enrich_df.to_csv(f, columns=[enrich_file], header=['count'], index=True, sep='\t')
